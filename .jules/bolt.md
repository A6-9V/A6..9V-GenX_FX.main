...
## 2026-02-14 - Pre-calculating Squares and Reducing Series Overhead
**Learning:** I identified a performance bottleneck in `utils/technical_indicators.py` where multiple rolling variance calculations (Bollinger Bands and multiple volatility periods) were independently calculating `close_vals**2`. Additionally, `_calculate_rolling_slope` was creating multiple intermediate `pd.Series` objects, triggering costly index alignment logic for every arithmetic operation.
**Action:** I pre-calculated `close_vals_sq` once at the start of `add_volatility_indicators` and reused it across all variance loops. I also refactored `_calculate_rolling_slope` to perform all arithmetic on raw NumPy arrays, creating a single `pd.Series` only for the final result. These optimizations provided a ~6.7% speedup for the `add_all_indicators` method (~1.0s vs ~1.07s for 200k rows), demonstrating that even small redundant calculations and Pandas overhead can accumulate in data-intensive paths.
