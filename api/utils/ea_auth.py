...
def get_valid_ea_api_keys() -> set:
    ...
    # Priority 1: Check environment variables directly (important for tests)
    env_ea_api_key = os.environ.get("EA_API_KEY")
    if env_ea_api_key:
        valid_keys.add(env_ea_api_key)

    env_ea_api_keys = os.environ.get("EA_API_KEYS")
    if env_ea_api_keys:
        keys = [k.strip() for k in env_ea_api_keys.split(",") if k.strip()]
        valid_keys.update(keys)

    # Priority 2: Use settings if nothing found in direct env check
    if not valid_keys:
