...
async def validate_ea_api_key(api_key: Optional[str] = Security(api_key_header)) -> str:
    ...
    if not valid_keys:
        # If no keys are configured, we allow access but log a warning
        # This maintains backwards compatibility with older EAs and tests
        if not api_key:
            logger.debug("EA API request without key (no keys configured)")
