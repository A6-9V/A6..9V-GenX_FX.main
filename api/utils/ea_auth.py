...
def get_valid_ea_api_keys() -> set:
    ...
    # ⚡ Bolt Optimization: Check os.environ directly for reactivity during tests.
    # The Pydantic settings object may be instantiated before test fixtures can
    # inject the necessary environment variables.
    ea_api_key = os.environ.get("EA_API_KEY")
    if ea_api_key:
        valid_keys.add(ea_api_key)

    ea_api_keys = os.environ.get("EA_API_KEYS")
    if ea_api_keys:
        keys = [k.strip() for k in ea_api_keys.split(",") if k.strip()]
        valid_keys.update(keys)

    # Fallback to settings if not in os.environ (though usually they are the same)
    if not valid_keys:
...
async def validate_ea_api_key(api_key: Optional[str] = Security(api_key_header)) -> str:
    ...
    # If no keys are configured, allow unauthenticated access for backward compatibility
    # and to support existing test suites.
    if not valid_keys:
