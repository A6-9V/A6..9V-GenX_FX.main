...
def get_valid_ea_api_keys() -> set:
    ...
    # ⚡ Bolt: Check environment directly to ensure reactivity during tests
    # where os.environ might be updated after settings are initialized.
    ea_api_key = os.getenv("EA_API_KEY") or settings.EA_API_KEY
    if ea_api_key:
        valid_keys.add(ea_api_key)

    ea_api_keys = os.getenv("EA_API_KEYS") or settings.EA_API_KEYS
    if ea_api_keys:
