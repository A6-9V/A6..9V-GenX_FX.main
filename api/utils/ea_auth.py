...
def get_valid_ea_api_keys() -> set:
    ...
    Checks both settings and os.environ for reactivity during tests.
    ...
    # Get directly from os.environ for test reactivity
    env_key = os.environ.get("EA_API_KEY")
    if env_key:
        valid_keys.add(env_key)

    env_keys = os.environ.get("EA_API_KEYS")
    if env_keys:
