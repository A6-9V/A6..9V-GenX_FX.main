...
def client():
    ...
    # Save original environment to avoid leaking state to other test modules
    old_api_key = os.environ.get("EA_API_KEY")
    old_api_keys = os.environ.get("EA_API_KEYS")
    ...
    if old_api_key is not None:
        os.environ["EA_API_KEY"] = old_api_key
    else:
        os.environ.pop("EA_API_KEY", None)

    if old_api_keys is not None:
        os.environ["EA_API_KEYS"] = old_api_keys
    else:
        os.environ.pop("EA_API_KEYS", None)
    ...
    def test_multiple_keys_config(self):
        ...
        try:
            ...
            if old_keys is not None:
                os.environ["EA_API_KEYS"] = old_keys
            else:
                os.environ.pop("EA_API_KEYS", None)
